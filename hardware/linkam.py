...
"""
Below is an adaptation of the microscope.stages.linkam module.
https://www.python-microscope.org/api/microscope.stages.linkam.html
This was adapted as part of a summer
internship (2020) for the Faraday Institution. Understanding the below
is assisted by reviewing the documentation for the LinkamSDK.

Contact:
Adam Alderton
aa816@exeter.ac.uk

Below is the original comment header, crediting the author Mick Phillips.
"""
...
ctypes objects.
  ...
  * errors are run in to when using 64-bit python environment, use 32-bit
  """
  
try:
    import win32api
except ImportError:
    import pywin32 as win32api
...
class Msg(IntEnum):
    ...
    #\param[in]      hDevice     A valid handle to a comms device/port returned by eLinkamFunctionMsgCode_OpenComms. (Above)
...
class _LinkamBase(devices.FloatingDeviceMixin, devices.Device):
    ...
    def get_sdk_version():
        ...
        __class__._lib.linkamGetVersion.restype = _bool_t
        success = __class__._lib.linkamGetVersion(b, _max_version_length)
        return success, b.value.decode()

    @staticmethod
    def init_sdk():
        # Open LinkamSDK.dll, assuming it is in current working directory. Ensure that os.add_dll_directory(os.getcwd()) has been called.
        __class__._lib = ctypes.CDLL("LinkamSDK.dll")
        _lib = __class__._lib

        # Assuming log can be created in working dir
        # Assuming linkam.lsk (licence file) is in working directory
        # linkam_log will be moved to inside experiment dir on completion of experiment
        sdk_log = ("linkam_log.txt").encode()
        lsk_path = ("linkam_license.lsk").encode()
        
        # Ensure return type of linkamInitiliaseSDK is bool, then initialise
        _lib.linkamInitialiseSDK.restype = _bool_t
        if (_lib.linkamInitialiseSDK(sdk_log, lsk_path, True)):
            _return = True
        else:
            _return = False
    ...
    def __del__(self):
        # On object deletion close comms
        self._close_comms()
    ...
    def check_connection(self):
        ...
        if not self._connectionstatus.flags.connected:
            ...
            for key in keys[keys.index("connected"):]:
                print("{} = {}".format(key, int(status[key])))
    ...
    def move_log(self, experiment_name):
        # If possible, move linkam_log.txt into the directory of the experiment
        try:
            try:
...
class _HeatingStage(_LinkamBase):
    # Base class for linkam stages that have heating functionalities
    def __init__(self, uid = "", **kwargs):
        # Initialise _LinkamBase class to inherit from
        super().__init__(**kwargs)

        # The below should be overwritten by a child class, but are here for generalisation
        self.min_temp = -200
        self.max_temp = 2000
        self.min_rate = 0.001
        self.max_rate = 500
        
        # Initialise the LinkamSDK
        if self.init_sdk():
            
            # Retrieve the version of the SDK
            version_success, version = self.get_sdk_version()
            if version_success:
                
                # Initialise USB connection, exception will be raised if it fails
                self.uid = uid
                self.init_usb(self.uid)
                self.check_connection()
                
            else:
                raise Exception("Could not get the version number of the LinkamSDK.")
        else:
            raise Exception("Could not initialise the LinkamSDK. Check the log file for details.")
        
        # Had problems with linkam log file, so have disabled logging past after initialisation
        self._process_msg(Msg.DisableLogging)
        
    def __del__(self):
        self.stop_heating()
    
    def _start_heating(self, temp, rate, holdtime):
        # Set rate, target temperature and holdtime
        self.set_value(_StageValueType.HeaterRate, rate)
        self.set_value(_StageValueType.HeaterSetpoint, temp)
        # In vtipy2, the holdtime used here is manually overruled, but is still implemented here for future functionality
        self.set_value(_StageValueType.RampHoldTime, holdtime)
        
        # Process StartHeating message. param1 = True instructs to turn ON
        self._process_msg(Msg.StartHeating, True)
    
    def stop_heating(self):
        self._process_msg(Msg.StartHeating, False)
        
    def get_temperature(self):
        return round(self.get_value(_StageValueType.Heater1Temp), 2)
    
    def get_holdtime_remaining(self):
        # Can be used to return the hold time remaining for a heating ramp, if the current state of the stage is holding.
        # Can also be used to detect when the stage is holding, as below will return 0.0 if stage is NOT holding.
        return self.get_value(_StageValueType.RampHoldRemaining)
    
    def toggle_hold(self):
        # Send this message to impose a manual hold on the temperature
        # Sending this message again will toggle the hold mode off
        self._process_msg(Msg.ForceHold)


class HFS350(_HeatingStage):
    # Class to encapsulate the functionality of the HFS350-PB4 stage from Linkam.
    # It inherents from the _HeatingStage class, providing heating functionality.
    # The HFS350 stage from Linkam also has vacuum capability, so functionality to 
    # automate that should be added here or perhaps in a "_VacuumStage" class to inherit from.
    
    def __init__(self, uid = "", **kwargs):
        super().__init__(**kwargs)

        # HFS350 specific parameters, largely taken from brochures and manuals. Performance could vary at the limits of these values.
        self.name = "HFS350"
        self.min_temp = -196
        self.max_temp = 350
        self.min_rate = 0.1
        ...
        # The tolerance (°C) within which (above and below) a temperature is "reached".
        self.tolerance = 1
    
    def update_tolerance(self, temp):
        # Updates the tolerance value above, dependent on temperature
        if temp <= 100.0:
            self.tolerance = 0.01
        elif 100.0 < temp <= 200:
            self.tolerance = 0.05
